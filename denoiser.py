...
def GlowDenoiser(args):
    ...
    for gamma in loopOver:
        skip_to_next  = True# flag to skip to next loop if recovery is fails due to instability
        n             = args.size*args.size*3
        modeldir      = "/content/drive/MyDrive/GlowIP-master/trained_models/%s/glow"%args.model
        test_folder   = "/content/drive/MyDrive/GlowIP-master/test_images/%s"%args.dataset
        save_path     = "/content/drive/MyDrive/GlowIP-master/results/%s/%s"%(args.dataset,args.experiment)
        ...
        psnr      = [peak_signal_noise_ratio(x, y) for x,y in zip(Original, Recovered)]
